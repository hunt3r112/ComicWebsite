...
<html lang="en">
    <?php
        ...
        $genre_query = "select * from genre where GenreID = $genreID";
        $genre = mysqli_fetch_assoc(mysqli_query($conn,$genre_query));
        $page = 1;
        $limit = 10;
        $arr_sql = "select ComicID from comic_genre where GenreID = $genreID";
        ...
        $start=($page-1)*$limit;
        $item_select_query = "select * from comic inner join comic_genre cg on comic.ComicID = cg.ComicID where GenreID = $genreID order by LastUpdated desc limit $start,$limit";
        $item_result = mysqli_query($conn, $item_select_query);
    ?>
    <head>
        <meta charset="UTF-8">
        <link rel="stylesheet" href="Css/stylesheet.css">
        <title>Thể loại: <?php echo $genre['GenreName'];?></title>
    </head>
    <body id="container">
    <?php
        ...
        <div id="main-wrapper">
            <div id="search-content-wrapper">
                <div class="section-header">
                    <h2 class="section-name new-comic-section">Thể loại: <?php echo $genre['GenreName'];?> » Tổng: <?php echo $total_record;?> truyện</h2>
                </div>
                        ...
                        for($i=1;$i<=$total_page;$i++)
                        {
                            ...
                            <li>
                                <a class="pagination-item <?php if($i == $page) echo "pagination-item-active";?>"
                                   href="genre.php?page=<?php echo $i ;?>">
                                    <?php echo $i; ?></a>
                            </li>
                            <?php
                        }
